...
def deploy_tensorflow_model(clipper_conn,
    ...
    if not serialization_dir:
        clipper_conn.build_model_func(
            name, version, input_type, func, base_image, labels,
...
def tensorflow_predict(clipper_conn,
                       headers, 
                       req,
                       base_image="default",
                       logger=None):
    """Get the tensorflow session running results. 
    ...
    clipper_conn : :py:meth:`clipper_admin.ClipperConnection`
        A ``ClipperConnection`` object connected to a running Clipper cluster.
    headers : str
        The header of rpc request. 
    req : str
        A json encoded string that contains the payload for session run.
    base_image : str, optional
        The base Docker image to build the new model image from. This
        image should contain all code necessary to run a Clipper model
        container RPC client.
    logger: ClusterAdapter, optional
        Log the debug information of containers. 
    """
    py_minor_version = (sys.version_info.major, sys.version_info.minor)
    if base_image == "default":
        if py_minor_version < (3, 0):
    ...
    if logger is not None:
        logger.start(base_image, headers, req)

    addr = clipper_conn.get_query_addr()
    url = "http://%s/tensorflow/predict" % addr
    r = clipper_conn.submit_query(url=url, headers=headers, data=req)

    if logger is not None:
        logger.end(base_image, headers, req)
